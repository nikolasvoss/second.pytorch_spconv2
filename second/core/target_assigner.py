...
class TargetAssigner:
    ...
    def assign_per_class(self,
                         ...
                         importance=None):
        """
        This Docstring was autogenerated.
        Assigns target labels and bounding box regression targets individually for each class.

        This function is recommended for multi-class networks where anchor targets are assigned
        according to the ground truth (gt) information provided.

        Parameters:
        - anchors_dict (dict): A dictionary containing anchor information per class.
                               Each entry should have keys "anchors", "matched_thresholds", and
                               "unmatched_thresholds".
        - gt_boxes (numpy.ndarray): A numpy array of shape (N, 7) representing the ground truth bounding boxes.
        - anchors_mask (numpy.ndarray, optional): A mask of shape (M,) for anchors indicating which ones to include or exclude.
        - gt_classes (numpy.ndarray, optional): A numpy array of shape (N,) representing the class ids of ground truth boxes.
        - gt_names (numpy.ndarray, optional): A numpy array of shape (N,) representing the names of classes for ground truth boxes.
        - importance (numpy.ndarray, optional): A numpy array of shape (N,) representing the importance of each ground truth box,
                                                used for weighting the loss associated with each box during training.

        Returns:
        - targets_dict (dict): A dictionary containing the following keys:
            - "labels": A numpy array of labels for each anchor, reshaped to (-1,)
            - "bbox_targets": Bounding box regression targets for each anchor, reshaped to (-1, self.box_coder.code_size)
            - "importance": An optional numpy array indicating the importance of each anchor, reshaped to (-1,)

        This method employs internal subfunctions such as `box_encoding_fn` for encoding the
        bounding boxes and `similarity_fn` for computing similarity between anchors and
        ground truth boxes for each class.

        The target assignment involves steps such as matching anchors to the closest ground truth box,
        pruning anchors based on the mask, applying matching and unmatched thresholds. The final targets
        include class labels, bounding box regression targets, and optionally, anchor importance weights.

        Note: This docstring assumes that some inner functions and class properties are known in advance,
        such as `self._box_coder`, `self._sim_calcs` among others, and this requires the reader to have
        prior knowledge or access to the class implementation details.
        """

        def box_encoding_fn(boxes, anchors):
            return self._box_coder.encode(boxes, anchors)

        targets_list = []  # List to store targets for each class
        anchor_loc_idx = 0 # Index for anchor locations
        anchor_gene_idx = 0 # Index for anchor generators
        # for each class, calculate the similarity between anchors and gt_boxes
        for class_name, anchor_dict in anchors_dict.items():

            def similarity_fn(anchors, gt_boxes):
                """
                Calculate similarity between anchors and ground truth boxes for a given class.
                Args:
                    anchors: (N,7) array of anchors
            ...
            # Generate boolean mask for ground truth names matching the current class
            mask = np.array([c == class_name for c in gt_names],
                            dtype=np.bool_)

            feature_map_size = anchor_dict["anchors"].shape[:3]
            # Number of locations for anchors
            num_loc = anchor_dict["anchors"].shape[-2]
            if anchors_mask is not None:
                # Reshape the anchors mask to match the total number of anchors
                anchors_mask = anchors_mask.reshape(-1)
                # Get anchor range for the current class
                a_range = self.anchors_range(class_name)
                # Apply the mask to select relevant anchors
                anchors_mask_class = anchors_mask[a_range[0]:a_range[1]].reshape(-1)
                # Define a pruning function for anchors using the class-specific mask
                prune_anchor_fn = lambda _: np.where(anchors_mask_class)[0]
            else:
                # No mask provided, so no pruning is needed
                prune_anchor_fn = None
            # print(f"num of {class_name}:", np.sum(mask))

            # Create ground truth targets for anchor boxes
            targets = create_target_np(
                anchor_dict["anchors"].reshape(-1, self.box_ndim),
                gt_boxes,#[mask], # only gt_boxes that match the current class
                similarity_fn,
                box_encoding_fn,
                prune_anchor_fn=prune_anchor_fn,
                gt_classes=gt_classes,#[mask], # only gt_classes that match the current class
            ...
            # Update index for location and generator for the next iteration
            anchor_loc_idx += num_loc
            targets_list.append(targets)
            anchor_gene_idx += 1

        # Combine targets from all classes into a single dictionary
        targets_dict = {
